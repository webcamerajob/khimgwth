...
# --- Функция для исторических данных через Visual Crossing ---
async def get_historical_records(lat: float, lon: float, api_key: str) -> Optional[Dict[str, float]]:
    """Получает исторические рекорды (мин/макс) для текущего дня через Visual Crossing."""
    if TEST_MODE:
        return {"record_min": 22.1, "record_max": 35.8}

    url = f"{VISUALCROSSING_API_URL}{lat},{lon}/today"
    params = {
        'unitGroup': 'metric',
        'key': api_key,
        'include': 'normal,days', # Запрашиваем normals и days
        'elements': 'tempmin,tempmax'
    }
    ...
    try:
        ...
        # ИСПРАВЛЕНО: Парсим данные из блока "normal"
        if 'days' in data and data['days'] and 'normal' in data['days'][0]:
            normals = data['days'][0]['normal']
            
            # Рекордный минимум - это первое значение в массиве tempmin
            # Рекордный максимум - это третье значение в массиве tempmax
            if 'tempmin' in normals and len(normals['tempmin']) == 3 and 'tempmax' in normals and len(normals['tempmax']) == 3:
                record_min = normals['tempmin'][0]
                record_max = normals['tempmax'][2]
                logger.info(f"Получены исторические рекорды для {lat},{lon}: min={record_min}, max={record_max}")
                return {"record_min": record_min, "record_max": record_max}
...
# --- Основные функции (с небольшими изменениями) ---
...
def get_random_background_image(city_name: str) -> str | None:
    ...
    logger.warning(f"Папка с фонами не найдена или пуста: {city_name}")
    return None
...
def create_weather_gif(frames: List[Image.Image], output_path: str = "weather.gif") -> str:
    if not frames:
        logger.error("Нет кадров для создания GIF.")
        return ""
    final_frames, durations = [], []
    transition_steps, hold_duration, blend_duration, num_colors = 15, 3000, 100, 128
    num_frames = len(frames)
    for i in range(num_frames):
        current_frame, next_frame = frames[i], frames[(i + 1) % num_frames]
        quantized_frame = add_watermark(current_frame.copy()).quantize(colors=num_colors, method=Image.Quantize.MEDIANCUT)
        final_frames.append(quantized_frame)
        for step in range(1, transition_steps + 1):
            blended = Image.blend(current_frame, next_frame, alpha=step/transition_steps)
            quantized_blended = add_watermark(blended).quantize(colors=num_colors, method=Image.Quantize.MEDIANCUT)
    ...
    logger.info(f"Оптимизированный GIF создан: {output_path}")
    return output_path
