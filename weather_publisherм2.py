...
import asyncio
import os
import json # Добавлено для работы с JSON файлами
from datetime import datetime, timedelta # Добавлено для отметок времени и расчетов времени удаления
...
# URL для AccuWeather API (постоянный, не относится к настройкам публикации/удаления)
ACCUWEATHER_BASE_URL = "http://dataservice.accuweather.com/"

---

## Настройки публикации и удаления

# --- Основные настройки ---
# Включить тестовый режим? (True = использовать предустановленные данные, False = использовать реальный API)
TEST_MODE = True
# Список городов, для которых будет публиковаться погода
CITIES_TO_PUBLISH = ["Пномпень", "Сиануквиль", "Сиемреап"]
# Файл для сохранения ID сообщений, которые нужно будет удалить
MESSAGES_TO_DELETE_FILE = "messages_to_delete.json"
# Время задержки перед удалением сообщений (в часах)
DELETE_AFTER_HOURS = 0.003

# --- Настройки вотермарки ---
# Имя файла вашей вотермарки (должен быть в корневой папке репозитория)
WATERMARK_FILE = "watermark.png"
# Масштаб вотермарки (например, 0.25 означает 25% от ширины основного изображения)
WATERMARK_SCALE_FACTOR = 0.3
...
# --- Настройки таймаутов ---
# Задержка между запросами к API AccuWeather (в секундах)
API_REQUEST_DELAY_SECONDS = 0.5
# Задержка между отправкой отдельных фотографий в Telegram (в секундах)
TELEGRAM_SEND_PHOTO_DELAY_SECONDS = 1

---

# --- Предустановленные данные о погоде (для TEST_MODE) ---
# Эти данные используются, когда TEST_MODE установлен в True.
# Они имитируют ответы AccuWeather API для каждого города.
PRESET_WEATHER_DATA = {
    "Пномпень": {
...
async def main():
    ...
    for city in CITIES_TO_PUBLISH: # Используем CITIES_TO_PUBLISH из настроек
    ...
    for city in CITIES_TO_PUBLISH: # Используем CITIES_TO_PUBLISH из настроек
        ...
        await asyncio.sleep(API_REQUEST_DELAY_SECONDS) # Задержка между запросами к API

    # Если есть сгенерированные изображения, отправляем их по одному
    if generated_image_paths:
        # Список для сохранения Message ID
        sent_message_ids = []
        ...
        for i, path in enumerate(generated_image_paths):
            try:
                ...
                with open(path, 'rb') as f:
                    ...
                    sent_message_ids.append(message.message_id) # Добавляем ID в список
                    logger.info(f"Фото {path} успешно отправлено. Message ID: {message.message_id}")
                
                await asyncio.sleep(TELEGRAM_SEND_PHOTO_DELAY_SECONDS) # Задержка между отправкой фото в Telegram
            except FileNotFoundError:
                logger.error(f"Файл изображения не найден: {path}. Пропускаю его.")
            except Exception as e:
                logger.error(f"Ошибка при отправке фото {path}: {e}")
        
        # --- БЛОК: Сохранение Message ID в файл ---
        if sent_message_ids:
            try:
                # Загружаем существующие ID, если файл существует
                existing_data = []
                if os.path.exists(MESSAGES_TO_DELETE_FILE):
                    with open(MESSAGES_TO_DELETE_FILE, 'r', encoding='utf-8') as f:
                        try:
                            ...
                            logger.warning(f"Файл {MESSAGES_TO_DELETE_FILE} поврежден или пуст. Создаю новый.")
                            existing_data = []
                
                # Добавляем новые ID к существующим
                new_entry = {
                    "timestamp": datetime.now().isoformat(), # Отметка времени отправки
                    "chat_id": target_chat_id,
                ...
                logger.info(f"Message IDs успешно сохранены в {MESSAGES_TO_DELETE_FILE}.")
            except Exception as e:
                logger.error(f"Ошибка при сохранении Message ID в файл: {e}")
